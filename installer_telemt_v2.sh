...
echo "⚠️Если у вас не внешний IP, т.е. от провайдера вы получатее IP первый октет(цифра) которого 10|100|172|192, то работать прокси будет только внутри вашей локальной сети.⚠️"
